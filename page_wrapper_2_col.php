...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableContent">
  	...
  	for($b=0; $b < $countItem; $b++)
	{
		...
		else
		{
			...
			if($itemsArr[$b]['ITEMAPPENDTOBEFORE'] == 1 && $appendTrueFlag == false)
			{
				...
				$tempStr = buildInput($myQuery, $itemsArr[$b], $b+1);

				$newItemsArr[$appendToItemNo]['ITEMNOTES'] .= $itemsArr[$b]['ITEMTITLE'].$tempStr.$itemnoteurl;
			}
    ...
    <td class="inputArea"><?php
		...
		//to build input
		$tempStr = buildInput($myQuery, $itemsArr[$a], $a+1);
