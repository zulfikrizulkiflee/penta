...
{
	//get item info
	$getItemInfo = "select a.ITEMID, a.ITEMLOOKUP, a.COMPONENTID
	...
	if($getItemInfoRsCount)
	{
		...
		//for filterby list
		$filterBy = $myQuery->query(convertDBSafeToQuery("select * from (".$lookup.") a"),'SELECT','NAME');
...
<form id="form1" name="form1" action="" method="post" style="padding:10px;">
	...
	<table align="center" border="0" cellpadding="3" cellspacing="0" class="tableContent" style="margin-left:0px;width:100%" >
		<tr>
			<th colspan="2">List of Values Filter</th>
		</tr>
		<tr>
			<td class="inputLabel">Filter List By : </td>
			<td>
				<select class="inputList" style="width:350px;" name="filterBy" id="filterBy" onchange="moveCursorToEnd(document.getElementById('value'));">
					<option selected="selected" value="">-- OPEN SEARCH --</option>               
					<?php 
					for($y=1; $y < $lookupRsKeysCount; $y++ )
					{
						...
						?>
						<option value="<?php echo $keyNameValue; ?>" <?php if($_POST['filterBy'] == $keyNameValue) echo 'selected'; ?>><?php echo $keyName ?></option>
						<?php
					}
					?>
				</select>
		...
		<tr>
			<td colspan="2" class="contentButtonFooter">
				...
				<input name="close" type="button" class="inputButton" value="Close" onclick="window.close()" />
			</td>
		</tr>
	</table>
	<br />
	<div style="height:370px; overflow:auto">
		<table width="100%" border="0" cellpadding="3" cellspacing="0" class="tableContent" style="margin-left:0px;" >
			<tr>
				<th colspan="<?php echo $lookupRsKeysCount; ?>">List of Values </th>
			</tr>
			<?php if($lovArrCount) { ?>
			<tr>
