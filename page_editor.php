...
if($_POST['saveScreenRefNew'])
{
	...
	*/
	...
	$newComponent_1 = "insert into FLC_PAGE_COMPONENT (COMPONENTID,COMPONENTNAME,COMPONENTTITLE,COMPONENTORDER,PAGEID,COMPONENTTYPE,
						...
						COMPONENTCOLLAPSE,COMPONENTCOLLAPSEDEFAULT,COMPONENTQUERYMAXFETCH";
	...
	$newComponent_2 = "values (".$maxComponentRs.",'".$_POST['newComponentName']."','".$_POST['newComponentTitle']."',
						...
						".$_POST['collapseDefault'].",".$_POST['newTabularMaxFetch'];
...
if($_POST['saveScreenItemNew'])
{
	...
	$newItem = "insert into FLC_PAGE_COMPONENT_ITEMS
				(ITEMID, ITEMNAME, ITEMTITLE, ITEMTYPE, ITEMORDER, ITEMHINTS, ITEMNOTES, ITEMINPUTLENGTH, ITEMTEXTAREAROWS,
					ITEMDEFAULTVALUE, ITEMDEFAULTVALUEQUERY, ITEMLOOKUPTYPE, COMPONENTID,
				...
				values (".$maxIDRs.", '".$_POST['newItemName']."', '".$_POST['newItemTitle']."', '".$_POST['newItemType']."',
					...
					".$_POST['newItemTextAreaRows'].", '".$_POST['newItemDefaultValue']."', ".$_POST['newItemDefaultValueQuery'].",
					'".$_POST['newLookupType']."', ".$_POST['componentID'].",
	...
	if($_POST['newItemType'] == 'chart' && $newItemRs)
	{
		...
		$insertChart = "insert into FLC_CHART (ITEM_ID, CHART_BG_COLOR, CHART_NO_PREFIX, CHART_DECIMAL_PRECISION,
							CHART_PTYPE, CHART_X_AXIS_LABEL, CHART_PY_AXIS_LABEL, CHART_PSHOW_VALUE,
							CHART_SY_AXIS_LABEL, CHART_SSHOW_VALUE, CHART_TREND_LABEL, CHART_TREND_VALUE, CHART_TREND_COLOR)
						values
							(".$maxIDRs.", '".$_POST['chartBgColor']."', '".$_POST['chartNoPrefix']."',
							".$_POST['chartDecimalPrecision'].", '".$_POST['chartPtype']."', '".$_POST['chartXAxisLabel']."',
							'".$_POST['chartPyAxisLabel']."', ".$_POST['chartPshowValue'].",
							'".$_POST['chartSyAxisLabel']."', ".$_POST['chartSshowValue'].",
...
else if($_POST['saveScreenItemEdit'])
{
	...
	$trigger_id= "select b.TRIGGER_ID from FLC_TRIGGER a, FLC_TRIGGER_PARAMETER b
						where a.TRIGGER_ID = b.TRIGGER_ID and a.TRIGGER_ITEM_ID=".$_POST['hiddenItemID']."
	...
	if(count($trigger_idRs) > 0)
	{
		...
		$deleteValidation = "delete from FLC_TRIGGER where TRIGGER_ITEM_ID = ".$_POST['hiddenItemID'];
...
else if($_POST['editItem'])
{
	...
	$parameter_value =explode('|',str_replace("'",'',$showValidateRs[0]['PARAMETER_VALUE']));
...
<form method="post" name="form1">
  ...
  <table width="100%" border="0" cellpadding="3" cellspacing="0" class="tableContent">
    ...
    <tr id="typeQueryRow" <?php if($getEditComponentRs[0]['COMPONENTTYPE']!='search_constraint' && $getEditComponentRs[0]['COMPONENTTYPE']!='query' && $getEditComponentRs[0]['COMPONENTTYPE']!='query_2_col' && $getEditComponentRs[0]['COMPONENTTYPE']!='report'){?> style="display:none;"<?php }?>>
      <td class="inputLabel">Query : </td>
      <td><textarea name="editTypeQuery" style="width:99%" rows="10" class="inputInput" id="editTypeQuery"><?php echo convertToDBQry($getEditComponentRs[0]['COMPONENTTYPEQUERY']);?></textarea>
        <br />
  ...
  <table width="100%" border="0" cellpadding="3" cellspacing="0" class="tableContent">
    ...
    <tr>
      <td class="inputLabel">Type : </td>
      <td>
      	<select name="newItemType" class="inputList" id="newItemType" onchange="codeDropDown(this, 'saveScreenItemNew'); changeItemType(this.value);">
          <option value="">&nbsp;</option>
    ...
    <tr id="row_itemDefault">
      ...
      <td>
		...
		<br />
		<label><input name="newItemDefaultValueQuery" type="checkbox" value="1" onchange="if(this.checked==true){swapItemDisplay('newItemDefaultValueQuery','newItemDefaultValueText')} else {swapItemDisplay('newItemDefaultValueText','newItemDefaultValueQuery')}" />
		Use SQL</label>
	  </td>
    </tr>
    ...
    <tr>
      ...
      <td>
          ...
          <br />
          <label><input name="newAdvQueryLimit" type="checkbox" id="newAdvQueryLimit" value="1" disabled="disabled" />Unlimited</label>
      </td>
    </tr>
    </tbody>
    <tr id="row_itemHints">
    ...
    <tbody id="additionalTbody" style="display:none;">
      ...
      <td>&nbsp;
      <div style="height:200px;width:35%;overflow:auto;">
     <?php
    ...
    <tr>
      ...
      <td>
          ...
          <option value="">&nbsp;</option>
          <option value="single_area">Area</option>
          <option value="single_bar">Bar</option>
          <option value="single_column">Column</option>
          <option value="single_doughnut">Doughnut</option>
          <option value="single_line">Line</option>
          <option value="single_pie">Pie</option>
    ...
    <tr>
      ...
      <td>
        ...
        <label><input name="chartPshowValue" type="checkbox" value="1" checked />Show Value</label>
      </td>
    </tr>
    ...
    <tr id="chartSsqlTr" style="display:none;">
      ...
      <td>
        ...
        <label><input name="chartSshowValue" type="checkbox" value="1" />Show Value</label>
      </td>
    </tr>
    </tbody>
  </table>
  ...
  <table width="100%" border="0" cellpadding="3" cellspacing="0" class="tableContent">
    ...
    <tr>
      <td class="inputLabel">Type : </td>
      <td>
      	<select name="editItemType" class="inputList" id="editItemType" onchange="codeDropDown(this, 'saveScreenItemEdit'); changeItemType(this.value);">
          <?php for($x=0; $x < count($itemTypeRs); $x++) { ?>
    ...
    <tr>
      <td class="inputLabel">Default Value : </td>
      <td>
	  	<input name="editItemDefaultValue" type="text" class="inputInput" id="editItemDefaultValueText" size="70" value="<?php echo $showEditComponentItemRs[0]["ITEMDEFAULTVALUE"];?>" <?php if($showEditComponentItemRs[0]["ITEMDEFAULTVALUEQUERY"]){?>disabled="disabled" style="display:none"<?php }?> />
		...
		<label><input name="editItemDefaultValueQuery" type="checkbox" value="1" <?php if($showEditComponentItemRs[0]["ITEMDEFAULTVALUEQUERY"]){?> checked="checked"<?php }?> onchange="if(this.checked==true){swapItemDisplay('editItemDefaultValueQuery','editItemDefaultValueText')} else {swapItemDisplay('editItemDefaultValueText','editItemDefaultValueQuery')}" />
		Use SQL</label>
	  </td>
    </tr>
    ...
    <tr>
      ...
      <?php $showEditComponentItemRs[0]['ITEMLOOKUP'] = str_replace('\\','',$showEditComponentItemRs[0]['ITEMLOOKUP']);
			?>
      <td><textarea name="editAdvancedLookup" cols="100" rows="10" class="inputInput" id="editAdvancedLookup" <?php if($showEditComponentItemRs[0]['ITEMLOOKUPTYPE'] != 'advanced') { ?>disabled="disabled"<?php } ?>><?php if($showEditComponentItemRs[0]['ITEMLOOKUPTYPE'] == 'advanced') echo $showEditComponentItemRs[0]['ITEMLOOKUP'];?></textarea><br>
...
<br>
  <label><input name="editAdvQueryLimit" type="checkbox" id="editAdvQueryLimit" value="1" <?php if($showEditComponentItemRs[0]['ITEMLOOKUPUNLIMITED']){?> checked="checked"<?php }?> <?php if($showEditComponentItemRs[0]['ITEMLOOKUPTYPE'] != 'advanced') { ?>disabled="disabled"<?php } ?> />Unlimited (fetch all rows)</label></td>
    </tr>
    </tbody>
    <tr id="row_itemHints">
      <td class="inputLabel">Hints : </td>
	...
	for ($i=0 ; $i<count($resultdesc) ; $i++)
		{
	//echo $resultdesc[$i]['REFERENCECODE'];
    ...
    <tr>
      ...
      <td>
          ...
          <option value="">&nbsp;</option>
          <option value="single_area" <?php if($getChartItemRs[0]['CHART_PTYPE'] == 'single_area'){?> selected="selected"<?php }?>>Area</option>
          <option value="single_bar" <?php if($getChartItemRs[0]['CHART_PTYPE'] == 'single_bar'){?> selected="selected"<?php }?>>Bar</option>
          <option value="single_column" <?php if($getChartItemRs[0]['CHART_PTYPE'] == 'single_column'){?> selected="selected"<?php }?>>Column</option>
          <option value="single_doughnut" <?php if($getChartItemRs[0]['CHART_PTYPE'] == 'single_doughnut'){?> selected="selected"<?php }?>>Doughnut</option>
          <option value="single_line" <?php if($getChartItemRs[0]['CHART_PTYPE'] == 'single_line'){?> selected="selected"<?php }?>>Line</option>
          <option value="single_pie" <?php if($getChartItemRs[0]['CHART_PTYPE'] == 'single_pie'){?> selected="selected"<?php }?>>Pie</option>
    ...
    <tr id="chartSsqlTr" <?php if($getChartItemRs[0]['CHART_PTYPE'] != 'combination_column_line'){?> style="display:none;"<?php }?>>
      ...
      <td>
        ...
        <br /><label class="labelNote">Note: Category Name will refer to Primary Chart. Rename the next column to be Series Name (Except Single Series Chart)</label><br />
        <label><input name="chartSshowValue" type="checkbox" value="1" <?php if($getChartItemRs[0]['CHART_SSHOW_VALUE']){?> checked<?php }?> />Show Value</label>
      </td>
    </tr>
    </tbody>
  </table>
