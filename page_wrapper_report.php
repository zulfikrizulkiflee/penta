		...
		switch($itemsArr[$a]['ITEMTYPE'])
		{
			...
			case 'timepicker':
				...
				for($ax=0;$ax<$dataCount;$ax++)
				{
					...
					$temp = convertInputIntoArray($itemsArr[$a]['ITEMTYPE'], buildInput($myQuery, $itemsArr[$a], $ax), $ax);
					...
					if($itemsArr[$a]['ITEMTYPE'] != 'hidden')
					{
						...
						else
						{
							...
							$data[$ax][$dataKeys[$a-1]] .=  ' '.$dataKeys[$a].' '.$temp.' '.$itemsArr[$a]['ITEMNOTES'];
						}//eof else
					}//eof if
					else
					{
			...
			default:
				...
				//build input
				$temp = buildInput($myQuery, $itemsArr[$a], $a+1);
				//$temp = convertInputIntoArray($itemsArr[$a]['ITEMTYPE'], buildInput($myQuery, $itemsArr[$a], $a+1), $ax);
