...
function insertComponent($parent,$pageType,$compType,$table,$mySQL,$myQuery)
{
	...
	{
		//add search constraint component
		if($compType == 'search_constraint')
		{
			$title = 'Maklumat Carian '.$table;
			$compType = 'search_constraint';
			$searchList = array();

			//get list of all columns
			for($x=0; $x < $_POST['colCount']; $x++)
			{
				//get selected columns for search constraint ONLY
				if($_POST['attr_search_'.$x])
				{
					//TODO
					//if(DBMS_NAME == 'sybase_ASE')
					//	$fromDualStr = "";
					//else
					//	$fromDualStr = "from dual ";

					$searchList[] = "select ''".$_POST['attr_search_'.$x]."'' as flc_id, ''".str_replace(' :','',$_POST['title_'.$x])."'' as flc_name ".$mySQL->fromDual();
			...
			$compQuery = "'".implode(' union all ',$searchList)."'";
			$compRowNo = 'null';
			$compAddRow = 'null';
			$compDelRow = 'null';
			$compAddRowDisabled = 'null';
			...
			//get minimum order
			$minOrder = $myQuery->query("select MIN(COMPONENTORDER) as MINN
									from FLC_PAGE_COMPONENT where PAGEID = ".$parent,'SELECT','NAME');		//get max component order

			$maxOrder = $minOrder[0]['MINN'];

			//update all component order by 1
			$updateAllCompOrder = $myQuery->query("update FLC_PAGE_COMPONENT set COMPONENTORDER = COMPONENTORDER+1
													where PAGEID = ".$parent,'RUN');
		}
		else
		{
			$title = 'Senarai '.$table;
			...
			else if(strtolower(DBMS_NAME) == 'oracle' || DBMS_NAME == 'sybase_ASE')
			{
	...
	$myQuery->query("insert into FLC_PAGE_COMPONENT
					...
					values (".$maxID.",'".$name."','".$title."',".$maxOrder.",'".$compType."',".$compQuery.",".$compRowNo.",
