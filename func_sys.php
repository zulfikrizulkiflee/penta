<?php
//ckm 20120308
function getLanguage($myQuery,$getVals)
{
	$qry = "select LANG_ID, LANG_NAME, LANG_FLAG_URL
			from FLC_TRANSLATION_LANGUAGE
			order by LANG_ID";
	...
	if(strpos($_SERVER['REQUEST_URI'],'?') == false)
	{
		for($x=0; $x < count($qryRs); $x++)
			$tempStr .= '<span style="cursor:pointer;"><a href="index.php?lang='.$qryRs[$x]['LANG_ID'].'"><img alt="'.$qryRs[$x]['LANG_NAME'].'" src="'.$qryRs[$x]['LANG_FLAG_URL'].'" /></a></span> ';
	}
	else
	{
		//all these to avoid duplicated GET var for lang
		$qryStrArr = $_GET;
		$qryStrArrKeys = array_keys($qryStrArr);
		$qryStrArrCount = count($qryStrArr);

		//loop on count of GET parameter
		for($x=0; $x<$qryStrArrCount; $x++)
		{
			if($qryStrArrKeys[$x] != 'lang' && !(URL_SECURITY&&$qryStrArrKeys[$x] == 'a'))
				$tempArr2[] = $qryStrArrKeys[$x].'='.$qryStrArr[$qryStrArrKeys[$x]];
		}//eof for

		//loop on count of language
		for($x=0; $x < count($qryRs); $x++)
		{
			if($tempArr2)
				$url = 'index.php?'.implode('&',$tempArr2).'&lang='.$qryRs[$x]['LANG_ID'];
			else
				$url = 'index.php?lang='.$qryRs[$x]['LANG_ID'];

			//if menu security enabled
			if(MENU_URL_SECURITY)
			{
				//if using index.php
				if(strtolower(substr($url,0,10)) == 'index.php?')
					$url = 'index.php?a='.flc_url_encode(substr($url,10));
			}//eof if

			$tempStr .= '<span style="cursor:pointer;"><a href="'.$url.'" title="'.$qryRs[$x]['LANG_NAME'].'"><img alt="'.$qryRs[$x]['LANG_NAME'].'" src="'.$qryRs[$x]['LANG_FLAG_URL'].'" /></a></span> ';
		}//eof for

		/*//all these to avoid duplicated GET var for lang
		$qryStrArr = explode('&',$_SERVER['QUERY_STRING']);
		$qryStrSplit = array();

		for($x=0; $x < count($qryStrArr); $x++)
		{
			$tempArr = explode('=',$qryStrArr[$x]);
		...
		for($x=0; $x < count($qryStrSplit); $x++)
			$tempArr2[] = $qryStrSplit[$x][0].'='.$qryStrSplit[$x][1];

		for($x=0; $x < count($qryRs); $x++)
			$tempStr .= '<span style="cursor:pointer;"><a href="index.php?'.implode('&',$tempArr2).'&lang='.$qryRs[$x]['LANG_ID'].'" title="'.$qryRs[$x]['LANG_NAME'].'"><img alt="'.$qryRs[$x]['LANG_NAME'].'" src="'.$qryRs[$x]['LANG_FLAG_URL'].'" /></a></span> ';*/
	}
	return $tempStr;
}
...
{
	//if languange is given
	if($language)
	{
		//set charset to utf8
		setCharSetUTF8($myQuery);

		/*$qry = "select * from FLC_TRANSLATION
				where TRANS_TYPE = 1
				and TRANS_LANGUAGE = ".$language;
		$qryRs = $myQuery->query($qry,'SELECT','NAME');

		//fallback
		$qry2 = "select * from FLC_TRANSLATION
				where TRANS_TYPE = 1
				and TRANS_LANGUAGE = 1";
		$qry2Rs = $myQuery->query($qry2,'SELECT','NAME');*/
...
function displayContent($dbc,$myQuery,$mySQL)
{
...
$insertAudit = "insert into FLC_AUDIT
						...
						AUDIT_REQUEST_URI, AUDIT_REQUEST_MENU_ID, AUDIT_ACTION, AUDIT_CONTROL, USER_ID)
						values (".$mySQL->currentDate().",
						'".$_SERVER['REMOTE_ADDR']."', '".gethostbyaddr($_SERVER['REMOTE_ADDR'])."', '".session_id()."',
						'".$requestURI."', ".$menuID.", '".$action."', '".$auditControl."', ".$userID.")";
*/
		$insertAuditRs = $myQuery->query($insertAudit,'RUN');
	}//eof if
...
//include extension
function includeExtension($extType, $fileMode, $itemArr = NULL)
{
	//db connection
	include('db.php');
...
function flc_mail($from,$to,$subject,$msg,$filename,$filearray)
{
	//---------------
	//php.ini example
	//---------------
	...
	*/
	...
	$countupload = count($uploadedFileArrName);
	...
	//attach the attachments to the message
		for($x=0; $x < $countupload; $x++)
		{
			...
			$content = fread($file,filesize($uploadedFileArrTmp[$x]));
			fclose($file);
...
function DisplayLanguageSelector($myQuery)
{
	...
	//all language
	$q = "select LANG_ID,LANG_NAME, LANG_FLAG_URL from FLC_TRANSLATION_LANGUAGE";
...
function LoadUserTheme($myQuery)
{
	...
	else //public theme/layout atau first landing pada login page
	{
		...
		$getThemeFolderRs = $myQuery->query("select THEME_FOLDER from FLC_THEME where THEME_ID = '".$getThemeRs."'",'SELECT','INDEX');
		$getThemeFolderRs = $getThemeFolderRs[0][0];
		$_SESSION['THEME_FOLDER'] = $getThemeFolderRs;
