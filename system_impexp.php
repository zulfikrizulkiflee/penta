...
else if($_POST['exportModule'])
{
	...
	writeToDump($timestamp,$getQry_FLC_MENU,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_PAGE,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_PAGE_COMPONENT,0,1,$fileExtension);				//write to file	
	...
	writeToDump($timestamp,$getQry_FLC_PAGE_COMPONENT_ITEMS,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_PAGE_CONTROL,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_CHART,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_TRIGGER,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_TRIGGER_PARAMETER,0,1,$fileExtension);			//write to file
	...
	{	
		//get the item
		$theItem = "select ".implode(',',$columnName)." from FLC_BL where BLNAME = '".$allBlName[$x]."'";
		$theItemRs = $myQuery->query($theItem,'SELECT','INDEX');
	...
	writeToDump($timestamp,$getQry_FLC_BL,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_BL_PARAMETER,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_PERMISSION,0,1,$fileExtension);				//write to file
	...
	writeToDump($timestamp,$getQry_FLC_EXTENDED_ATTR,0,1,$fileExtension);	
	...
	writeToDump($timestamp,$getQry_FLC_EXTENDED_ATTR_VAL,0,1,$fileExtension);				//write to file
...
else if($_POST['importModule'])
{
	...
	$primaryKeyToUpdate = array('MENUID','PAGEID','COMPONENTID','ITEMID','CONTROLID','ITEM_ID','TRIGGER_ID','BLID','GROUP_ID','PERM_ID');
	...
	//UPDATE CHART
	$chartIDListArr = explode(',',$chartIDList);
	$cnt = ceil(count($chartIDListArr)/$inLimit); 
	
	if($chartIDList != '' && count($chartIDListArr) > 0)
	{		
		for($x=0; $x < $cnt; $x++)
		{
			$updateChartContent = "update FLC_CHART set
										...
										where ITEM_ID in (".implode(',',array_slice($chartIDListArr,($x*$inLimit),$inLimit)).")";
