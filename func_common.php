...
{
	if(!$state)
		$_SESSION['toggleState'][$menu] = true;
	else if($state)
	{
		$_SESSION['toggleState'][$menu] = false;

		//close all sub menu
		include('db.php');

		//get sub menu
		$getSubMenu = "select MENUID from FLC_MENU where MENUPARENT = ".$menu;
		$getSubMenuRs = $myQuery->query($getSubMenu, 'SELECT', 'NAME');
		$getSubMenuRsCount = count($getSubMenuRs);

		//loop on count of sub menu
		for($x=0; $x < $getSubMenuRsCount; $x++)
		{
			//if sub menu state true
			if($_SESSION['toggleState'][$getSubMenuRs[$x]['MENUID']])
			{
				//toggle menu state to false
				toggleMenu($getSubMenuRs[$x]['MENUID'],true);
			}//eof if
		}//eof for
	}//eof elseif
}//eof function
...
//build menu list
function build_menu_list($menu_array,$level=1,$open=false)
{
  /*//kalau bukan topmenu
  ...
  //apply style
  echo '<ul class="pureCssMenu sideMenuList menuLevel'.$level.'" style="'.$styleString.'">';
  ...
  foreach((array)$menu_array as $m)
  {
  	...
  	if($m['MENUSTATUS'] == '1')
  	{
  		...
  		if(is_array($m['MENUCHILD'])) {
  		  ...
  		  $link = 'href="'.$m['MENULINK'].'&menuID='.$m['MENUID'].'" ';
  		  $class = "noChild";
  		}
  		...
  		if($m['MENULEVEL'] == $level)
  		{
  		  ...
  		  /*if($_SESSION['LAYOUT'] != '3')
  		  {
  		  	if($level > 1) $stylePadding = 'padding-left:'.(15*$level).'px';
  		  }*/

  		  $label = ($class=='noChild' ? $m['MENUTITLE'] : '<span>'.$m['MENUTITLE'].'</span>');

  		  echo
  		  '<li  class="pureCssMenui">'.
  		  //.$icon_div.
  		  '<a class="pureCssMenui '.$class.'" id="menu_id_'.$m['MENUID'].'" '.$link.' style="'.$stylePadding.'" target="'.$m['MENUTARGET'].'">'.$label.'</a>';
  		  if( is_array($m['MENUCHILD']) )
  		  {
  		    $is_open = ($_SESSION['toggleState'][$m['MENUID']] == 1 ? true : false);
  		    build_menu_list((array)$m['MENUCHILD'],$level+1,$is_open);
  		  }
  		  echo '</li>';
  		}
  	}
