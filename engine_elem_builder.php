...
function buildInput($myQuery, $itemArr, $runningNo='', $style='')
{
	...
	switch($type)
	{
		...
		case 'auto_sugg':
			...
			$temp .= '<input name="auto_text_'.$name.'" id="auto_text_'.$name.'" type="text" class="'.$cssInputClass.'"  size="'.$length.'"
						title="'.$hints.'" placeholder="'.$placeholder.'" value="'.$label.'" onkeyup="flc_auto_suggest_lookup(this.value,'.$id.',\''.$name.'\')" />';

			$temp .= '<div id="suggestions_'.$id.'" style="position:absolute; box-shadow:#888 5px 10px 10px; -webkit-box-shadow : #888 5px 10px 10px;
		...
		case 'chart':
			...
			if($getChartRsCount>0)
			{
				...
				if($getChartRs[0]['CHART_PSQL'])
				{
					...
					$chartDataRs = $myQuery->query($chartData,'SELECT','NAME');
					...
					if($chartDataRsCount>0)
					{
						...
						if($getChartRs[0]['CHART_SSQL'])
						{
							...
							$chartSecondaryDataRs = $myQuery->query($chartSecondaryData,'SELECT','NAME');
		...
		case 'color_picker':
			...
			//value to return
			$returnValue = '<input name="'.$name.'" id="'.$name.'" type="'.$type.'" tabindex="'.$tabindex.'" maxlength="'.$maxchar.'" class="color {hash:true} '.$cssInputClass.'" value="'.$default.'" size="'.$length.'" style="'.$css.' " '.$js.' '.$disabled.$readonly.' title="'.$hints.'" />';
		break;
		...
		case 'date':
			...
			//value to return
			$returnValue = '<input name="'.$name.'" id="'.$name.'" type="text" tabindex="'.$tabindex.'" class="'.$cssInputClass.'" value="'.$default.'" size="'.$length.'" '.$js.' '.$disabled.$readonly.' title="'.$hints.'" placeholder="'.$placeholder.'" style="'.$css.'" /><script>var opts = {formElements:{"'.$itemId.'":"'.$dateFormat.'"}, showWeeks:true, statusFormat:"l-cc-sp-d-sp-F-sp-Y"}; datePickerController.createDatePicker(opts);</script>';
		break;
		...
		case 'hidden':
			...
			//value to return
			$returnValue = '<input name="'.$name.'" id="'.$name.'" type="'.$type.'" value="'.$default.'" size="'.$length.'" />';
		break;
		...
		case 'lov':
			...
			//text to show
			$temp .= '<input name="lov_text_'.$name.'" id="lov_text_'.$name.'" type="text" class="'.$cssInputClass.'" readonly="readonly" '.$cssReadonlyStyle.' value="'.$label.'" size="'.$length.'" title="'.$hints.'" placeholder="'.$placeholder.'" />';
		...
		case 'password':
			//value to return
			$returnValue = '<input name="'.$name.'" id="'.$name.'" type="password" tabindex="'.$tabindex.'" maxlength="'.$maxchar.'" class="'.$cssInputClass.'" value="'.$default.'" size="'.$length.'" '.$js.' '.$disabled.$readonly.' title="'.$hints.'" placeholder="'.$placeholder.'" />';
		break;
		...
		case 'plugin':
			//=============== INCLUDE EXTENSION ==============
			includeExtension('item', 'builder', $itemArr);
		...
		case 'text':
			...
			//value to return
			$returnValue = '<input name="'.$name.'" id="'.$name.'" type="'.$type.'" tabindex="'.$tabindex.'" maxlength="'.$maxchar.'" class="'.$cssInputClass.'" value="'.$default.'" size="'.$length.'" style="'.$css.'" '.$js.' '.$disabled.$readonly.' title="'.$hints.'" placeholder="'.$placeholder.'" />';
		break;
		...
		case 'textarea':
			...
			//value to return
			$returnValue ='<textarea name="'.$name.'" id="'.$name.'" tabindex="'.$tabindex.'" class="'.$cssInputClass.'" cols="'.$length.'" rows="'.$textarearows.'" style="'.$css.'" '.$js.' '.$disabled.$readonly.' title="'.$hints.'" placeholder="'.$placeholder.'">'.$default.'</textarea>';
		break;
		...
		case 'text_editor':
			...
			if($compType!='tabular')
				$itemId = 'CKEDITOR.replace(\''.$name.'\');';
			else
				$itemId = 'CKEDITOR.replace(\''.$name.'\');';

			//value to return
			$returnValue = '<textarea name="'.$name.'" id="'.$name.'" title="'.$hints.'">'.$default.'</textarea><script>'.$itemId.'</script>';
		break;
		...
		case 'timepicker':
			...
			$returnValue .= '<select id="timepicker_h_'.$name.'" style="width:50px;" class="inputList" title="HH" onchange="collectTimepickerValues(\''.$name.'\')">';
			...
			//minutes
			$returnValue .= ' :<select id="timepicker_m_'.$name.'" style="width:50px;margin-left:5px;" class="inputList"  title="MM" onchange="collectTimepickerValues(\''.$name.'\')"><option>&nbsp;</option>';
			...
			{
				//secs
				$returnValue .= ' :<select id="timepicker_s_'.$name.'" style="width:50px;margin-left:5px;" class="inputList"  title="SS" onchange="collectTimepickerValues(\''.$name.'\')"><option>&nbsp;</option>';
			...
			{
				$returnValue .= '<select  id="timepicker_ampm_'.$name.'" style="width:55px;margin-left:5px;" class="inputList" title="AM/PM" onchange="collectTimepickerValues(\''.$name.'\')">';
				$returnValue .= '<option>&nbsp;</option>';
			...
			*/
			...
			$returnValue .= '<input name="'.$name.'" id="'.$name.'" type="hidden" value="'.$default.'" '.$js.' />';
